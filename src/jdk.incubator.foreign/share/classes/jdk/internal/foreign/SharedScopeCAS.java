 ...
 */
public class SharedScopeCAS extends ResourceScopeImpl {
    ...
    private final SharedResourceList resourceList = new SharedResourceList();

    static {
        try {
            STATE = MethodHandles.lookup().findVarHandle(SharedScopeCAS.class, "state", int.class);
        } catch (Throwable ex) {
            throw new ExceptionInInitializerError(ex);
        }
    }

    SharedScopeCAS(Cleaner cleaner) {
        if (cleaner != null) {
            var list = resourceList;
            cleaner.register(this, () -> list.cleanup());
        }
    }
    ...
    public void addCloseAction(Runnable runnable) {
        addCloseActionInternal(this, runnable);
    }


    @ScopedMemoryAccess.Scoped
    @ForceInline
    static void addCloseActionInternal(SharedScopeCAS sharedScopeCAS, Runnable runnable) {
        try {
            sharedScopeCAS.checkValidState();
            sharedScopeCAS.resourceList.add(ResourceListSup.ResourceCleanup.ofRunnable(runnable));
        } catch (ScopedAccessError ex) {
            throw new IllegalStateException("Already closed!");
        } finally {
            Reference.reachabilityFence(sharedScopeCAS);
        }
    }
     ...
     */
    public abstract static class ResourceListSup implements Runnable {
        ResourceCleanup fst;
     ...
     */
    static class SharedResourceList extends ResourceListSup {

        static final VarHandle FST;

        static {
            try {
                FST = MethodHandles.lookup().findVarHandle(ResourceListSup.class, "fst", ResourceCleanup.class);
            } catch (Throwable ex) {
